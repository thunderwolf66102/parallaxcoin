...
int64 nChainStartTime = 1397344244;
...
const string strMessageMagic = "ParallaxCoin Signed Message:\n";
...
{
    // ParallaxCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 60 * 60; // ParallaxCoin: every hour
static const int64 nTargetSpacing = 1.5 * 60; // ParallaxCoin: 1.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // ParallaxCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, int nHeight, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // ParallaxCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // ParallaxCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // ParallaxCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ParallaxCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ParallaxCoinMiner : ProcessBlock, block not accepted");
    }
